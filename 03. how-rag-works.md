...
| ``<#>`` | Cosine distance | ``sql ``SELECT ``id, ``document_text ``FROM ``techcare_docs ``ORDER ``BY ``embedding_vector ``<#> ``'[0.11, ``-0.52, ``0.77, ``...]' ``LIMIT ``3;`` | Semantic similarity (best for embeddings) (direction matters more than magnitude) |
